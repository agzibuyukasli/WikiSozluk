📘 WikiSözlük – Konya Üniversite Öğrencileri için Sosyal & Akademik Platform

WikiSözlük, Konya’daki üniversite öğrencilerinin akademik destek, sosyal yaşam, şehir keşfi, staj & burs duyuruları gibi ihtiyaçlarını tek bir platformda buluşturan öğrenci odaklı bir uygulamadır.

Bu proje, Konya Gençlik Meclisi – DIVIZYON tarafından düzenlenen Jam Week (Açık Bilgi Platformu Jam) kapsamında geliştirilmiştir.

🌟 Özellikler
📚 Akademik Paylaşım Alanı

Ders notları

Özetler

Çalışma kaynakları

Öğrenciden öğrenciye akademik destek

🎉 Sosyal Yaşam & Etkinlikler

Konya’daki etkinliklerin duyurulması

Topluluk buluşmaları

Kulüp aktiviteleri

📍 Şehir Keşfi

Gezilecek yerler

Sevilen kafeler

Öğrenciler tarafından önerilen mekanlar

💼 Staj & Burs Duyuruları

Öğrenciler için güncel staj fırsatları

Burs ve proje destek duyuruları

🪙 Rozet Sistemi & Coin Ekonomisi

Kullanıcıların yaptığı her eylem coin kazandırır:

Yorum yapmak

Paylaşım yapmak

Beğenmek

Etkinlik önermek

Coin miktarına göre kullanıcı rozet kazanır ve seviye atlar:

🔹 Yeni Gelen
🔹 Meraklısı
🔹 Gezgin
🔹 Seyyah
🔹 Kaşif
🔹 Konya Bilgesi

💳 Coin → Genç Kültür Kart Puanı Dönüşümü

Platformda kazanılan coinler, Konya Genç Kültür Kart puanına dönüştürülebilir.

Öğrenciler bu puanları:

🍔 Yemek

🛒 Alışveriş

🎟️ Etkinlik

☕ Kafe harcamaları

gibi Genç Kültür Kart’ın anlaşmalı olduğu yerlerde kullanabilir.

🔐 Admin Paneli

Yönetici ekranı aşağıdaki yetkilere sahiptir:

Öneri ve paylaşımları onaylama/red

Paylaşımları düzenleme

Coin ödül oranlarını değiştirme

Kullanıcı eylemlerini analiz etme

🔑 Kullanıcı Giriş Yöntemleri

Kullanıcılar uygulamaya iki farklı şekilde giriş yapabilir:

🎓 edu uzantılı üniversite maili ile

💳 Genç Kültür Kart ID’si ile

🛠️ Kullanılan Teknolojiler

Proje tamamen Figma ve Cursor AI kullanılarak geliştirilmiştir.

Tasarım: Figma

Geliştirme ortamı: Cursor AI

Ön yüz yapısı: HTML / CSS / JavaScript tabanlı

Kod entegrasyonu: AI-assisted (Yapay zekâ destekli geliştirme)

📂 Proje Yapısı
WikiSozluk/
│── assets/
│── components/
│── pages/
│── styles/
│── scripts/
│── index.html
│── README.md

🚀 Kurulum (Opsiyonel)

Repo’yu klonlayın:

git clone https://github.com/agzibuyukasli/WikiSozluk.git


Projeyi çalıştırın:

Herhangi bir canlı server (VS Code Live Server, Cursor Preview vb.) ile açabilirsiniz.

🤝 Katkı

Projeye katkıda bulunmak isterseniz:

Forklayın

Yeni branch oluşturun

Değişiklikleri yapın

Pull request gönderin

🎉 Teşekkürler

Bu projenin ortaya çıkmasında destek olan:

Konya Gençlik Meclisi – DIVIZYON

Jam Week organizasyon ekibi

Tüm ekip arkadaşlarımız

hepsine teşekkür ederiz!
